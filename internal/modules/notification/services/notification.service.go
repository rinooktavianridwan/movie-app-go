...
func (s *NotificationService) CreatePromoNotification(userID uint, promoName string, promoCode string, promoID uint) error {
    req := &requests.CreateNotificationRequest{
        UserID:  userID,
        Title:   "New Promo Available!",
        Message: fmt.Sprintf("Check out our new promo: %s. Use code: %s", promoName, promoCode),
...
func (s *NotificationService) CreateBookingConfirmationNotification(userID uint, transactionID uint, movieTitle string, totalAmount float64) error {
    req := &requests.CreateNotificationRequest{
        UserID:  userID,
